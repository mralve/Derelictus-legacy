...
{
    public class BSpritePanel : BerryWindow
    {
        BItemField newField;
                ...
                if (newField != null)
                {
                    if (XCPManager.currentXCP != null)
                    {
                        ...
                        if (XCPManager.currentXCP.xpcMaps != null)
                        {
                            ...
                            newField.items = new FieldItem[XCPManager.currentXCP.spriteTextures.Length];
                            for (int i = 0; i < newField.items.Length; i++)
                            {
                                ...
                                newField.items[i].itemIcon = XCPManager.PngToSprite(XCPManager.currentXCP.spriteTextures[i]);
        ...
        {
            BSpriteEditor SEditor = new BSpriteEditor();
            SEditor.isSubWindow = true;
            SEditor.TargetSprite = handler.itemTarget.index;
            WindowManager.CreateWindow(0, 0, SEditor, true);
        }

        public void CreateSpriteImporter()
        {
            BImporter spriteImporter = new BImporter();
            spriteImporter.isSubWindow = true;
            spriteImporter.singelFileMode = true;
            spriteImporter.fileTypeTarget = ".png";
            spriteImporter.ImportCallBack = Import;
            WindowManager.CreateWindow(0, 0, spriteImporter, true, false, true);
        }
